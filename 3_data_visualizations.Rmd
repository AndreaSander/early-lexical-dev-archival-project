...
x<- plot_model(lmm_viz_aditive, type="eff", terms = c("age_months[14,20, 26, 32, 38, 44, 48] ", "exp_target_lang_prop[1 , 0.65, 0.39]"), line.size = 0.8, colors = c("#feb24c", "#d7301f", "#7f0000"),
...
levels(x$data$group_col)<- c("0-39", "40-65", "66-100")
...
```{r}
tc <- arch_re_zero%>%
  mutate(exp_bins = case_when(exp_target_lang_prop >= 0.1 & exp_target_lang_prop <=.39 ~"0-39",
                              exp_target_lang_prop >= .40 & exp_target_lang_prop <=.65 ~"40-65",
                              exp_target_lang_prop >=.66 & exp_target_lang_prop <=1 ~"66-100")) %>%
...
```{r}
#Plotting real data with predicted data
library(magick)
