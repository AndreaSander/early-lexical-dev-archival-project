...
#load(here("R Analyses/Eye_data/arch_final.Rda"))
...
load(here("arch_final_2.Rda"))

arch_final <- arch_final %>%
  select (-c(recording_name, subject_id))

final <- final %>%
  select(-c(recording_name, subject_id))

#all the unmatched rows arew on learn. There must be a problem in the anonimizing data set. Check
```
